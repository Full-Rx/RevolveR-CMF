...
if( in_array( ROLE, ['Admin', 'Writer'] ) ) {
  ...
  if( isset(SV['p']) ) {
    ...
    if( count( $comments_moderate ) > 0 && !$session_control ) {

      $comments_all = iterator_to_array(

        $RKI->Model::get('comments', [
      ...
      foreach( $comments_moderate as $n => $m ) {
        ...
        if( isset($m[ 1 ]) ) {
          ...
          $RKI->Notify::set('notice', 'Comment '. $m[ 1 ] .' erased.', null);
            ...
            foreach( $comments_all as $comment ) {
              ...
              if( (int)$comment['id'] === (int)$cmd[ 0 ] ) {  
                ...
                if( $comment['published'] !== $cmd[ 1 ] ) {
                  ...
                  $update['content'] = $RKI->HTML::markup( 
                  ...
                  $RKI->Model::set('comments', $update);

                  if( (bool)$cmd[ 1 ] ) {

                    $RKI->Notify::set('active', 'Comment '. $comment['id'] .' published.', null);

                  }
                  else {

                    $RKI->Notify::set('inactive', 'Comment '. $comment['id'] .' unpublished.', null);
    ...
    if( $usl ) {
      ...
      $RKI->Model::set( 'users', $user_to_drop );

      $RKI->Notify::set('inactive', 'Authorization canceled for user '. $user_id .'.', null);
  ...
  $contents .= '<ul>';
  $contents .= '<li class="revolver__tabs-tab-tab_1 revolver__tabs-tab" data-link="tab-0">'. $RNV->lang['Users'] .'</li>';
  $contents .= '<li class="revolver__tabs-tab-tab_2 revolver__tabs-tab activetab" data-link="tab-1">'. $RNV->lang['Comments moderation'] .'</li>';
  $contents .= '</ul>';
  ...
  if( $users ) {
    ...
    foreach( $users as $user ) {
      ...
      $contents .= '<h2>• '. $RNV->lang['User'] .' :: '. $user['nickname'] .'<span style="float:right;">[ <a title="'. $RNV->lang['Manage account'] .'" href="/user/'. $user['id'] .'/edit/">'. $RNV->lang['manage'] .'</a> ]</span></h2>';

      $contents .= '<img class="revolver__user-info-avatar" src="'. ( $user['avatar'] === 'default' ? '/public/avatars/default.png' : $user['avatar'] ) .'" alt="'. $user['nickname'] .' '. $RNV->lang['User profile'] .'" />';
      ...
      $contents .= $RKI->HTMLForm::build( $session_form_params );
  ...
  if( $comments ) {
    ...
    foreach( array_reverse($comments) as $c ) {
      ...
      $contents_comment .= '<h2>• '. $RNV->lang['Comment'] .' '. $c['comments']['id'] .' '. $RNV->lang['by'] .' <span>'. $c['comments']['user_name'] .'</span></h2>';
      $contents_comment .= '<time>'. $c['comments']['time'] .'</time>';

      $contents_comment .= '</header>';

      $contents_comment .= '<figure class="revolver__comments-avatar">';
      $contents_comment .= '<img src="'. $avatar .'" alt="'. $RNV->lang['Account avatar'] .' '. $c['comments']['user_name'] .'" />';
      $contents_comment .= '</figure>';

      $contents_comment .= '<div class="revolver__comments-contents">';

      $contents_comment .= $RKI->HTML::markup(
      ...
      $contents_comment .= '<label id="label_publish_'. $n .'">'. $RNV->lang['Published'] .':';
      ...
      $contents_comment .= '<label id="label_delete_'. $n .'">'. $RNV->lang['Delete comment'] .':';
    ...
    $contents .= $RKI->HTMLForm::build( $moderation_form_params );
