...
if( $flag_pass ) {
	...
	if( $user_old_password === $RKI->Cipher::crypt('decrypt', $user_password) ) {
		...
		if( $revolver_user_new_password === $revolver_user_confirm_password ) { 
			...
			$RKI->Notify::set('inactive', 'Passwords not mutch. Stored old password');
	...
	$RKI->Notify::set('status', 'Profile updated');

}

$title = $RNV->lang['User profile'] .' • '. $user_name;
...
$form_parameters_tab_1_html .= '<p>'. $RNV->lang['User Name'] .': <i>'. $user_name .'</i></p>';
$form_parameters_tab_1_html .= '<p>'. $RNV->lang['User Email'] .': <i>'. $user_email .'</i></p>';
$form_parameters_tab_1_html .= '<p>'. $RNV->lang['Telephone'] .': <i>'. $telephone .'</i></p>';
$form_parameters_tab_1_html .= '<p>'. $RNV->lang['Permissions'] .': <i>'. $user_permissions .'</i></p>';
						...
						'label_5' => [
							...
							'fields' => [
								...
								0 => [
									...
									'required'		=> true,
									'value'			=> $RKI->Cipher::crypt( 'decrypt', $user_password )
