...
if( $action === 'message' ) {
	...
	if( defined('form_pass') ) {
		...
		if( $allow_send && form_pass === 'pass' && (bool)SV['p']['identity']['validity'] ) {
			...
			$notification = '<p>'. $RNV->lang['Hello'] .' '. $mailTo .'! '. $RNV->lang['You have unreaded private message here'] .' <a href="'. site_host .'/user/messages/">here</a>.</p>';

			$RKI->Email::send(

				$user_m['email'], $RNV->lang['New private message'], $notification

			);

			$RKI->Notify::set('status', 'Message sent');

		}
		else {

			$RKI->Notify::set('notice', 'Message not sent. User not found');
...
if( $action === 'cleanup' && (bool)SV['p']['identity']['validity'] ) {
	...
	foreach( $messages_to_delete as $message ) {
		...
		if( count($mfiles) > 0 ) {
			...
			$RKI->Model::erase('messages_files', [
	...
	$RKI->Notify::set('notice', 'Messages deleted');
...
if( $messages_list ) {
	...
	foreach( $messages_list as $key => $v ) {
		...
		$message_files  = '<label>'. $RNV->lang['Attached Files'];
		...
		if( $mfiles ) {
			...
			foreach( $mfiles as $f ) {
				...
				$message_files .= '<li>';
				$message_files .= '#'. ( $message_files_count + 1 ) .' '. $RNV->lang['file path'] .': <a href="'. $attached_file .'" target="_blank" class="external">'. $f['file'] .'</a>';
		...
		$checkBox  = '<label>'. $RNV->lang['Delete message'] . ( (bool)$message_files_count ? ' '. $RNV->lang['and'] .' '. $RNV->lang['Attached Files'] : '') .': <input type="checkbox" name="revolver_message_'. $messages_counter .'" value="'. $v['id'] .'" /></label>'; 
		...
		$contents_messages .= '<header><b>'. $RNV->lang['Message from'] .' '. $v['from'] .'</b> <time>'. $v['time'] .'</time></header>';

		$contents_messages .= '<div class="revolver__messages-text">'. $RKI->HTML::Markup( 
...
$node_data[ 0 ] = [
	...
	'title'		=> $RNV->lang['Private messages'],
	'id'		=> 'user-messages',
	'route'		=> '/user/messages/',
	'contents'	=> $contents,
	'teaser'	=> null,
