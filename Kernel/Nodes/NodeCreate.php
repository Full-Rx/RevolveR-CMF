...
if( $RNV->auth ) {
	...
	if( in_array(ROLE, ['Admin', 'Writer'], true) ) {
		...
		if( isset(SV['p']) ) {
			...
			if( isset(SV['p']['revolver_country_code']) ) {
				...
				if( (bool)SV['p']['revolver_country_code']['valid'] ) {
					...
					$hreflang = $RKI->Language::getLanguageData( $index_language )['hreflang'];
			...
			if( defined('form_pass') ) {
				...
				if( $passed && form_pass === 'pass' && (bool)SV['p']['identity']['validity'] ) {
					...
					header( 'Location: '. $RNV->host . $node_route .'?notification=node-created' );

				}
				else {

					$RKI->Notify::set('notice', 'Route exist or security check not pass');
		...
		foreach( $RKI->Language::getLanguageData('*') as $country => $c ) {
			...
			$form_parameters['tabs']['tab_4']['fieldsets']['fieldset_4']['labels']['label_'. $labels_count]['title:html'] = $RNV->lang['Language'] .' <span class="revolver__stats-system">[ '. $c['code_length_3'] .' :: '. $c['code_length_2'] .' :: '. $c['hreflang'] .' ]</span> <span class="state-attribution laguage-list-item revolver__sa-iso-'. strtolower( $c['code_length_2'] ) .'"></span>'. $RNV->lang['contents country'] .' <span class="revolver__stats-country">['. $c['name'] .']</span>';
