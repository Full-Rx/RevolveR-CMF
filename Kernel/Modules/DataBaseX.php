...
final class DBX {
	...
	function __construct( iterable $db, File $file ) {
			...
			case 'in':
				...
				if( self::$strict ) {
					...
					if( $insert ) {
						...
						$SQL = 'INSERT INTO `'. self::innerEscape( (string)self::$sql_hash_table ) .'` ('. rtrim($SQL_1, ', ') .') VALUES ('. rtrim( $SQL_2, ', ' ) .');';
	...
	protected static function getHashes( iterable $tables ): iterable {
		...
		foreach( $tables as $table ) {
			...
			$data = self::$file::readFile( $hashFile );
			...
			if( $data ) {
				...
				// Put hash into file
				self::$file::saveFile($hashFile, $hash);
	...
	protected static function stmtSQL( string $sql, ?bool $m ): ?iterable {
		...
		if( (bool)self::$dbx_lnk[ 1 ]['CONNECTION'] ) {
			...
			if( self::$strict ) {
				...
				self::$dbx_lnk[ 0 ]->real_query( 'SET SESSION sql_mode=\'STRICT_ALL_TABLES\';' );

			} 
			else {

				self::$dbx_lnk[ 0 ]->real_query( 'SET SESSION sql_mode=\'\';' );
			...
			if( self::$sql_action === 'create' ) { 
				...
				self::$dbx_lnk[ 0 ]->real_query('SET default_storage_engine=INNODB;');

				self::$dbx_lnk[ 0 ]->real_query('SET GLOBAL innodb_file_format=Barracuda;');

				self::$dbx_lnk[ 0 ]->real_query(

					$sql

				);

				self::$dbx_lnk[ 0 ]->real_query('INSERT INTO `hash__'. self::innerEscape( self::$sql_hash_table ) .'` (`id`, `hash`) VALUES (\'1\', \'0\')');
	...
	public static function getCachesList( ?bool $mode = null ): iterable {
		...
		if( is_readable( $cache_dir ) ) {
			...
			foreach( self::$file::getDir( $cache_dir ) as $v ) {
	...
	protected static function getCacheFile( string $f ): ?string {
		...
		$data = self::$file::readFile( $f );

		if( $data ) {

			$cache = self::$encrypted_cache ? self::$cipher::crypt( 'decrypt', $data ) : $data;
	...
	protected static function saveCacheFiles( iterable $r, string $f ): void {
		...
		if( !preg_match('/revolver__index/i', $f) && !preg_match('/revolver__statistics/i', $f) && count($r) > 0 ) {
	...
	protected static function saveCacheFile( string $r, string $f ): void {
		...
		self::$file::saveFile($f, (self::$encrypted_cache ? self::$cipher::crypt('encrypt', $r) : $r));
