							...
							case 'feedback':
								...
								if( isset(SV['p']['revolver_feedback_message_message']) ) {
									...
									if( (bool)SV['p']['revolver_feedback_message_message']['valid'] ) {
										...
										$message = $RKI->HTML::Markup( SV['p']['revolver_feedback_message_message']['value'] );
								...
								if( $title && $message && $sender_name && $sender_email ) {
									...
									$RKI->Notify::set('active', '<div>'. etranslations[ $ipl ]['Sense avalibale for feedback'] .'.</div>', null);
									...
									$RKI->Notify::set('inactive', '<div>'. etranslations[ $ipl ]['Preview not available'] .'.</div>', null);
		...
		case '#feedback':
			...
			if( INSTALLED ) {
							...
							foreach( $feedback_moderate as $n => $m ) {
								...
								if( isset($m[ 1 ]) ) {
									...
									if( $feedbacks ) {
										...
										foreach( $feedbacks as $f ) {
											...
											if( (int)$m[ 1 ] === (int)$f['id'] ) {
												...
												$RKI->Model::erase('feedback_files', [
									...
									if( $files_deleted ) {
										...
										$RKI->Notify::set('notice', 'Feedback files erased');
										...
										foreach( $feedbacks as $f ) {
											...
											if( (int)$f['id'] === (int)$cmd[ 0 ] ) {  
												...
												if( $f['message_processed'] !== $cmd[ 1 ] ) {
													...
													$RKI->Model::set('feedback', $update);

													if( (bool)$cmd[ 1 ] ) {

														$RKI->Notify::set('active', 'Feedback '. $comment['id'] .' processed.', null);

													}
													else {

														$RKI->Notify::set('inactive', 'Feedback '. $comment['id'] .' unprocessed.', null);
					...
					if( form_pass === 'pass' ) {
						...
						if( $installed && $extension_enabled ) {
							...
							if( $message_title && $message_text && $sender_name && $sender_email && !$action_process ) {
								...
								$RKI->Notify::set('status', 'Feedback request stored and send');
					...
					if( form_pass === 'pass' ) {
						...
						if( (bool)$extension_uninstall_set ) {
							...
							if( ROLE === 'Admin' ) {
								...
								$RKI->Notify::set('notice', 'Extension feedback now uninstalled');
							...
							if( ROLE === 'Admin' ) {
								...
								if( (bool)$extension_install_set ) {
									...
									$RKI->Notify::set('status', 'Extension feedback now installed');
								...
								if( (bool)$extension_enabled_set && !(bool)$extension_enabled ) {
									...
									$RKI->Notify::set('status', 'Extension feedback now enabled');
									...
									if( ROLE === 'Admin' ) {
										...
										if( (bool)$extension_cache_enabled ) {
											...
											if( (bool)$extension_cache_enabled_set ) {
												...
												$RKI->Notify::set('status', 'Extension feedback cache enabled');

												$RKI->Notify::set('active', 'Extension feedback cache now active');
										...
										if( !(bool)$extension_cache_enabled && !(bool)$extension_cache_enabled_set ) {
											...
											$RKI->Notify::set('notice', 'Extension feedback cache not enabled');

											$RKI->Notify::set('inactive', 'Extension feedback cache now inactive');
								...
								if( !(bool)$extension_enabled_set && (bool)$extension_enabled ) {
									...
									$RKI->Notify::set('notice', 'Extension feedback now disabled');
				...
				if( !$installed ) {
					...
					$RKI->Notify::set('warning', 'Extension feedback not intsalled');

					if( ROLE !== 'Admin' ) {

						$RKI->Notify::set('status', 'Use Admin privileges to manage');
				...
				if( $installed && !$enabled ) {
					...
					if( ROLE === 'Admin' ) {
						...
						$RKI->Notify::set('inactive', 'Use Admin privileges to manage');
				...
				if( $installed && $enabled && !Auth && !$stored ) {
					...
					$contents .= $RKI->HTMLForm::build( $form_parameters_request_form, null, etranslations );
				...
				if( in_array(ROLE, ['Admin', 'Writer']) && $installed ) {
					...
					if( $feedback ) {
						...
						foreach( $feedback as $f ) {
							...
							$contents_feedback .= $RKI->HTML::markup( 
