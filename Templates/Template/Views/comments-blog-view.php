...
if( $comments ) {
	...
	foreach( $comments as $c ) {
		...
		$RKI->Template::$b[] = '<article itemprop="comment" itemscope itemtype="https://schema.org/Comment" id="comment-'. $c['id'] .'" class="revolver__comments comments-'. $c['id'] .' '. $class .'">';
		...
		$RKI->Template::$b[] = '<span>'. floor($crate) .'</span> / <span>5</span> #<span class="closest">'. count($crating) .'</span>';
		$RKI->Template::$b[] = '</div>';
